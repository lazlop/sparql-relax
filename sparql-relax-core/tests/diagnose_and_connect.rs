...
fn diagnoses_the_wrong_predicate_as_a_culprit() {
    ...
    let diagnosis = diagnose(BROKEN_QUERY, &store, 1, None, false, 0, false).unwrap();
...
fn diagnose_errors_with_timeout_when_even_the_original_query_cant_run_in_time() {
    ...
    // "no culprits found".
    let store = test_store();
    let result = diagnose(BROKEN_QUERY, &store, 1, Some(Duration::from_nanos(1)), false, 0, false);
...
fn finds_a_real_forward_forward_path_and_fixes_the_query() {
    let store = test_store();
    let report = diagnose_and_connect(BROKEN_QUERY, &store, 1, Some(4), Some(5), None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
...
fn connects_a_culprit_triple_whose_variable_is_not_in_the_select_list() {
    ...
    let report = diagnose_and_connect(query, &store, 1, Some(4), Some(5), None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert_eq!(report.original_row_count, 0);
    assert_eq!(report.results.len(), 1);
...
fn query_with_no_broken_triples_reports_no_culprits() {
    ...
    "#;
    let diagnosis = diagnose(query, &store, 1, None, false, 0, false).unwrap();
...
fn sample_limit_carries_along_the_original_query_s_own_rows() {
    ...
    "#;
    let diagnosis = diagnose(query, &store, 1, None, false, 5, false).unwrap();
    assert_eq!(diagnosis.original_row_count, 1);
...
fn sample_limit_caps_below_the_full_row_count() {
    ...
    "#;
    let diagnosis = diagnose(query, &store, 1, None, false, 3, false).unwrap();
    assert_eq!(diagnosis.original_row_count, 0);
...
fn reports_no_connection_when_depth_is_too_small() {
    let store = test_store();
    let report = diagnose_and_connect(BROKEN_QUERY, &store, 1, Some(1), Some(5), None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert_eq!(report.results.len(), 1);
    let result = &report.results[0];
...
fn pruned_row_count_respects_result_limit() {
    ...
    let unbounded = diagnose_and_connect(query, &store, 1, Some(4), Some(5), None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert_eq!(unbounded.results[0].pruned_row_count, 2);

    let capped = diagnose_and_connect(query, &store, 1, Some(4), Some(5), Some(1), NamespaceScope::Unrestricted, None, None, false, false).unwrap();
...
#[test]
fn expand_nonempty_results_false_skips_the_ablation_search_once_rows_exist() {
    // Two sensors: sensor1 (72) fails the FILTER, sensor2 (2000) passes it,
    // so the query already returns one row (sensor2) even though the FILTER
    // is genuinely excluding sensor1. With the default
    // `expand_nonempty_results: false`, that's never checked at all — the
    // ablation search is skipped outright once the original query is known
    // to return something.
    let store = Store::new().unwrap();
    store
        .load_from_slice(
    ...
    let skipped = diagnose(query, &store, 1, None, false, 0, false).unwrap();
    assert_eq!(skipped.original_row_count, 1);
    assert!(skipped.filter_culprits.is_empty(), "the search should never have run");
    assert!(skipped.culprits.is_empty());

    let expanded = diagnose(query, &store, 1, None, false, 0, true).unwrap();
    assert_eq!(expanded.original_row_count, 1);
    assert_eq!(expanded.filter_culprits.len(), 1, "opting in should find the same filter culprit as the empty-result case");
    assert_eq!(expanded.filter_culprits[0].row_count_without_filter, 2);
}
...
fn combines_distinct_paths_from_different_bound_pairs_as_alternatives() {
    ...
    let report = diagnose_and_connect(query, &store, 1, Some(4), Some(5), None, NamespaceScope::Unrestricted, None, None, false, true).unwrap();
    assert_eq!(report.results.len(), 1);
    let result = &report.results[0];
...
fn sample_limit_none_samples_every_distinct_bound_pair() {
    ...
    let capped = diagnose_and_connect(query, &store, 1, Some(2), Some(5), None, NamespaceScope::Unrestricted, None, None, false, true).unwrap();
    assert_eq!(capped.results[0].triples[0].hop_alternatives.len(), 5, "capped sampling stops at the limit");

    let uncapped = diagnose_and_connect(query, &store, 1, Some(2), None, None, NamespaceScope::Unrestricted, None, None, false, true).unwrap();
    assert_eq!(
...
fn depth_1_finds_nothing_but_depth_2_finds_a_joint_two_triple_culprit() {
    ...
    let depth_1 = diagnose(query, &store, 1, None, false, 0, false).unwrap();
    assert!(depth_1.culprits.is_empty(), "no single triple's removal alone unblocks the query");

    let depth_3 = diagnose(query, &store, 3, None, false, 0, false).unwrap();
    ...
    let report = diagnose_and_connect(query, &store, 3, Some(4), None, None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert_eq!(report.results.len(), 1);
    let result = &report.results[0];
...
fn does_not_connect_when_the_subject_is_unconstrained_elsewhere() {
    ...
    let report = diagnose_and_connect(query, &store, 1, Some(1), None, None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert_eq!(report.results.len(), 1);
    let connected_triple = &report.results[0].triples[0];
    assert!(connected_triple.hop_alternatives.is_empty(), "one-sided endpoints aren't searched");
    assert!(report.results[0].connected_query.is_none());
...
fn default_max_depth_is_2_for_pair_search() {
    ...
    "#;
    let pair_report = diagnose_and_connect(pair_query, &store, 1, None, Some(5), None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
...
fn namespace_scope_restricts_path_search_to_allowed_prefixes() {
    ...
    let unrestricted = diagnose_and_connect(query, &store, 1, Some(1), None, None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    ...
    let brick_restricted = diagnose_and_connect(query, &store, 1, Some(1), None, None, NamespaceScope::default(), None, None, false, false).unwrap();
    assert!(
...
fn a_disconnected_reduced_pattern_is_reported_as_a_cartesian_risk_not_silently_ruled_out() {
    ...
    let diagnosis = diagnose(DISCONNECTED_QUERY, &store, 1, None, false, 0, false).unwrap();
...
fn diagnose_and_connect_does_not_hang_or_error_on_a_disconnected_query() {
    let store = disconnected_store();
    let report = diagnose_and_connect(DISCONNECTED_QUERY, &store, 1, None, None, None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
...
fn ignore_cartesian_risk_lets_connect_attempt_a_combo_diagnose_would_otherwise_skip() {
    ...
    let guarded = diagnose_and_connect(DISCONNECTED_QUERY, &store, 1, None, None, None, NamespaceScope::Unrestricted, None, None, false, false).unwrap();
    assert!(guarded.results.is_empty(), "sanity check: the guard leaves nothing to connect");

    let unguarded = diagnose_and_connect(DISCONNECTED_QUERY, &store, 1, None, None, None, NamespaceScope::Unrestricted, None, None, true, false).unwrap();
...
fn ignore_cartesian_risk_recovers_the_real_culprit_diagnose_skipped() {
    let store = disconnected_store();
    let diagnosis = diagnose(DISCONNECTED_QUERY, &store, 1, None, true, 0, false).unwrap();
...
fn pruned_query_text_removes_the_given_triple_and_the_result_runs() {
    ...
    let rows = diagnose(&pruned, &store, 1, None, false, 0, false).unwrap();
...
fn an_optional_only_shared_variable_does_not_mask_a_cartesian_risk_in_the_required_pattern() {
    ...
    let diagnosis = diagnose(DISCONNECTED_WITH_OPTIONAL_BRIDGE_QUERY, &store, 1, None, false, 0, false).unwrap();
